...
def main():
   ...
   # Pivot to get variants side by side
    ...
    # Get all algo_name from augmented_df
    algo_names = augmented_df["algo_name"].unique()


    # Melt into long format
    melted = pivot.melt(
        id_vars=['System', 'Nodes', 'buffer_size'],
        value_vars=algo_names,
        var_name='Comparison',
        value_name='bandwidth_mean'
    )
    ...
    if args.plot_type == "violin":
        ...
        ax= sns.boxplot(data=melted, x='System', y='bandwidth_mean', hue='Comparison', showmeans=True)
        ...
        # Swarm plot
        ax = sns.swarmplot(data=melted, x='System', y='bandwidth_mean', hue='Comparison', dodge=True, alpha=0.5)
