...
def process_benchmark_file(file_path, warmup_ratio=0.2):
    ...
    for col, items in df.items():
        ...
        items = items.values.astype(np.float64)
        ...
        percentile_10 = np.percentile(items, 10)
        percentile_25 = np.percentile(items, 25)
        percentile_75 = np.percentile(items, 75)
        percentile_90 = np.percentile(items, 90)
        ...
        outliers = items[(items < (percentile_25 - 1.5 * iqr))
                        | (items > (percentile_75 + 1.5 * iqr))]
        ...
        stats = stats | {
            ...
            f'{col}/max': max_val,
            f'{col}/n_iter': n_iter,
            f'{col}/percentile_10': percentile_10,
            f'{col}/percentile_25': percentile_25,
            f'{col}/percentile_75': percentile_75,
            f'{col}/percentile_90': percentile_90,
            f'{col}/iqr': iqr,
            f'{col}/standard_error': standard_error,
            f'{col}/ci_lower': ci_lower,
            f'{col}/ci_upper': ci_upper,
            # 'outliers': list(outliers),
            f'{col}/num_outliers': num_outliers
        }
    return stats
